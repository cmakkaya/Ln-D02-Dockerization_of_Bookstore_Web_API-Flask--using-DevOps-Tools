...
resource "aws_instance" "cumhur-docker-lesson" {
    ...
    user_data = <<-EOF
          ...
          TOKEN="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
          FOLDER="https://$TOKEN@raw.githubusercontent.com/cmakkaya/bookstore-repo/main/"
...
resource "aws_security_group" "cumhur-docker-lesson" {
...
output "website" {
    value = "http://${aws_instance.cumhur-docker-lesson.public_dns}"
}